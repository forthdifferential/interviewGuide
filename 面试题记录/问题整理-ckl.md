...
<img src="images_typora/image-20240425000839596.png" alt="image-20240425000839596" style="zoom:50%;" />
...
哈希表：查找只适用于等值查询，不适用于范围查找，Memory用
...
<img src="images_typora/image-20240425010628651.png" alt="image-20240425010628651" style="zoom: 67%;" />
...
只需要通过索引就可以返回查询所需要的数据，而不必在查到索引之后再去查询数

为了去掉回表的性能损耗，建议**只查索引列**，这样告别了回表
...
MySQL 5.6 发布后针对扫描二级索引的一项优化改进。总的来说是通过把**索引过滤条件下推到存储引擎**，来减少 MySQL 存储引擎访问基表的次数（`减少回表次数`）以及 MySQL 服务层访问存储引擎的次数



**适用**：

InnoDB`和 `MyISAM ； range`、 `ref`、 `eq_ref`、`ref_or_null； 二级索引；
...
**不用的时候**： 引擎层获取满足索引键值的数据，在**server层用where语句过滤** 

-->> 先索引前缀匹配，找到A开头的，然后全部回表，删除掉不满足23的记录

**用索引下推**： 引擎层先根据过滤条件中包含的索引键确定索引记区间，再在这个区间的记录上使用**包含索引键的其他过滤条件进行过滤**，之后规避掉不满足的索引记录，只根据满足条件的索引记录回表取回数据上传到 MySQL 服务层。

server层对接收到的数据，使用 where 子句中不包含索引列的过滤条件做最后的过滤，然后返回数据给客户端。

-->> 索引前缀屁屁额，**找到A开头的，并且去掉不满足23的记录**
...
返回SELECT语句检索的`行数`。 结果是一个bigint值

1. count(*)和count(1)是一样的，找到表头，计算行数即可，不需要读取数据列的数据内容（没有Where情况）

2. count(列名),**去除掉NULL值的行数**，需要读取每一行的值。

   > count(*)是比count(列名)快。
   >
   > 没有Where和GroupBy字句中，count(*)在MyISAM中是有单独字段记录行数，在InnoDB中可以选一个**最小的非聚簇索引**快速查记录行数
...
1. 开闭原则：一个类应该对外扩展开放（继承、组合），对外修改关闭

2. 单一职责：一个类应该只有一个对他引起变化的原因

3. 里氏替换：子类复写父类的方法时，需要实现父类方法的职责，避免调用父类方法的地方出错。

   > 用在虚函数的重写，比如父类方法有个判断超时，子类记得也要加上，再做其他额外的

4. 接口隔离：用限定符private public protected限制对外的访问权限，不强迫客户依赖他们不用的接口；一个类对另一个类的依赖应该建立在最小的接口上

5. 迪米特原则： 最小知道原则，尽量降低类与类之间的耦合度

   > 不该有直接依赖关系的类之间不要有依赖； 必须有依赖关系的类，尽量只保持必要的接口（把依赖的抽象出子类来）

6. 依赖倒置： 高层模块不能依赖低层模块，两者都应该依赖抽象。

   > 也就是说设计的时候，抽象出一个模型，确定稳定点是抽象模型（比如说reactor模型），低层和高层都依赖抽象来具体开发。

7. 组合优于继承： 继承的耦合度太高， 组合解耦



高内聚，低耦合： **相近的功能**放在一个类当中，不相近的功能不要放在一个类中；类和类的依赖关系简单清晰，**依赖少**点



#### 开闭原则简述，其他原则和他的关系

一个类应该对外扩展开放（继承、组合），对外修改关闭

我理解单一职责 里氏替换 接口隔离 都是这个衍生出来的相关
